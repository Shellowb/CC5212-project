...
AS (date:datetime, avg:float, agvu:float, city:chararray, country:chararray,  la:chararray, lo:chararray);
...
max_min = FOREACH group_city_year GENERATE FLATTEN($0) AS (country, year), MIN($1.avg) AS min_T, MAX($1.avg) AS max_T;

delta_T_by_city = FOREACH max_min GENERATE (max_T - min_T) AS delta_T

STORE delta_T_by_city into 'hdfs://cm:9000/user/uhadoop/uhadoop2020/grupo08/project/results/';